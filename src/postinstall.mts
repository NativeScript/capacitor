...
// @ts-ignore
import xcode from 'nativescript-dev-xcode';
          ...
          if (fs.existsSync(nsTsConfigPath)) {
            ...
            if (nsTsConfigContent) {
              ...
              try {
                ...
                nsTsConfigJson = ts.parseConfigFileTextToJson(
                  'tsconfig.json',
                  nsTsConfigContent
                ).config;
              }
  ...
  if (fs.existsSync(tsConfigPath)) {
    ...
    if (tsConfigContent) {
      ...
      try {
        ...
        // fallback as above can cause issues on some setups
        tsConfigJson = ts.parseConfigFileTextToJson('tsconfig.json', tsConfigContent).config;
      }
      if (tsConfigJson) {
...
function installIOS(): Promise<void> {
  const projectPath = path.join(rootPath, xcodeProjName),
        ...
        if (podfileContent && podfileContent.indexOf("pod 'NativeScript'") === -1) {
          if (podfileContent) {
            ...
            if (assertDeploymentPostInstallIndex > -1) {
              ...
              modifyPartPods.splice(
                assertDeploymentPostInstallIndex + assertDeploymentPostInstall.length + 1,
                0,
                `\n  ${podFilePostInstallStep}\n`
              );
