...
if (typeof (window as any).native === 'undefined') {
  ...
  const native_handler = {
    get: function (target: any, prop: any, receiver: any) {
      if (prop === 'isAndroid') {
        return (window as any).native.__func.__android;
      } else if (prop === 'isIOS') {
        return (window as any).native.__func.__ios;
      }
      if (prop === '__func') {
        return target;
  ...
  class NativeScriptProxy {
    ...
    constructor() {
      ...
      (this as any)[NS_PROXY] = proxy;
      (this as any)[NS_PROXY_FUNCTION] = proxy.__func;

      // Setup the Listener from NativeScript
      (NativeScriptCap as any).addListener('fromNativeScript', (info: any) => {
      ...
      if (msg.tracking === -1) {
        switch (msg.cmd) {
          case NS_MARSHALL_PLATFORM:
            ...
            if (msg.platform) {
              (window as any).native.__func.__android = true;
            } else {
              (window as any).native.__func.__ios = true;
            }
            break;
    ...
    getProxy(parent: any, prop: any, proxyClass: any) {
      ...
      (func as any)[NS_PROXY_ID] = this.getNextId();
      (func as any)[NS_PROXY_PROP] = prop;
      (func as any)[NS_PROXY_CHILDREN] = {};
      (func as any)[NS_PROXY_IS_FUNCTION] = parent == null;
      (func as any)[NS_PROXY_HAS_WAITING_VALUE] = false;

      //nativeLog("Prop", prop, parent == null ? "Null" : "Has Parent", func[NS_PROXY_IS_FUNCTION] ? "Func" : "Good");

      if (parent) {
        (func as any)[NS_PROXY_PARENT] = parent;
        (func as any)[NS_PROXY_FULL_PROP] =
          parent[NS_PROXY_FULL_PROP] + '.' + prop;
        parent[NS_PROXY_CHILDREN][prop] = func;
      } else {
        (func as any)[NS_PROXY_FULL_PROP] = prop;
      }

      // Create Proxy Object
      const newProxy = new Proxy(func, proxyClass);
      (func as any)[NS_PROXY] = newProxy;
  ...
  (window as any).nativeLog = function (...args) {
  ...
  (window as any).native = new NativeScriptProxy();
}

export const native: typeof nativeAPI & customNativeAPI = (window as any).native;
export const nativeLog: nativeLogType = (window as any).nativeLog;
