...
const buildNativeScript = () => {
  ...
  // const configPath = import.meta.resolve(`@nativescript/capacitor/bridge/webpack.config.js`);
  ...
  const url = new URL(`../bridge/${webpackInfo.name}`, import.meta.url);
...
if (!installOnly) {
  ...
  if (args && !args.find(a => a.indexOf('env=distFolder') > -1)) {
