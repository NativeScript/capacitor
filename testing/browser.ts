...
class NativeScriptProxy {
        ...
        if (callbacks.func[NS_PROXY_IS_FUNCTION]) {
            ...
            if (callbacks.func[NS_FUNCTION_PENDING] === 0) {
                while (callbacks.func[NS_FUNCTION_PENDING_LIST].length) {
                    let func = callbacks.func[NS_FUNCTION_PENDING_LIST].pop();
                    func();
                }
            }
        }
    ...
    fixArgs(value: any) {
        return new Promise(async (resolve) => {
            ...
            if (Array.isArray(value)) {
                ...
                let idx = (++this._callbackId);
                let callId = "__NS_CALLBACK__"+idx;
    ...
    getProxy(parent: any, prop: any, proxyClass: any) {
        ...
        if (parent) {
            ...
            (<any>func)[NS_PROXY_FULL_PROP] = parent[NS_PROXY_FULL_PROP] + "." + prop;
    ...
    _getter(target: any, prop: any, _receiver: any) {
        if (DEBUGGING) {
            if (prop !== "__func" && prop !== '__isProxy' && prop !== 'then') {
...
(<any>window).native = (<any>new NativeScriptProxy());
//(<any>window).native = native;

function nativeLog(...args: any): void {
    // eslint-disable-next-line prefer-rest-params
    const results = Array.prototype.join.call(args, " ");
    console.log("native.Log: ", results);
    //NativeScriptCap.notify( {value: JSON.stringify({cmd: NS_MARSHALL_CONSOLE, log: results})});
}
